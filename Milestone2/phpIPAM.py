...
class phpIPAM(object):
    ...
    def login(self):
        p = requests.post(self.base + 'user/',
        ...
        logging.debug("phpipam.login: IPAM Ticket: %s"
                      % (ticketJson['data']['token']))
        logging.debug("phpipam.login: IPAM Ticket expiration: %s"
                      % (self.token_expires))

    """ check if a ticket is still valid"""
    def ticket_check(self):
        headers = {'token': self.token}
        p = requests.get(self.base + 'user/', headers=headers, verify=False)
        if p.status_code != 200:
            logging.error("phpipam.ticket_check: Invalid ticket relogging in")
            logging.error(p.text)
            self.login()
        logging.info("phpipam.ticket_check: IPAM Ticket expiration: %s"
                     % (self.token_expires))

    """ Although the ticket last 6 hours. You can extend the time """
    def ticket_extend(self):
        headers = {'token': self.token}
        requests.patch(self.base + 'user/', headers=headers, verify=False)
        logging.info("phpipam.ticket_extend: IPAM Ticket expiration: %s"
                     % (self.token_expires))
    ...
    def authorization(self, controller):
        headers = {'token': self.token}
        p = requests.options(self.base+"%s/"
                             % (controller), headers=headers, verify=False)
        auth_json = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.authorization: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.authorization: success %s"
                     % (auth_json['success']))
    ...
    def sections_get_all(self):
        headers = {'token': self.token}
        p = requests.get(self.base + "sections/?links=false",
                         headers=headers, verify=False)
        sections_get_all_json = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.sections_get_all: Failure %s"
                          % (p.status_code))
        ...
        if not sections_get_all_json['success']:
            logging.error("phpipam.sections_get_all: FAILURE: %s"
                          % (sections_get_all_json['code']))
            self.error = sections_get_all_json['code']
            return self.error

        logging.info("phpipam.sections_get_all: success %s"
                     % (sections_get_all_json['success']))
    ...
    def sections_get_id(self, section):
        headers = {'token': self.token}
        p = requests.get(self.base + "sections/%s/?links=false"
                         % (section), headers=headers, verify=False)
        sections_get_id_json = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.sections_get_id: Failure %s"
                          % (p.status_code))
        ...
        if not sections_get_id_json['success']:
            logging.error("phpipam.sections_get_id: FAILURE: %s"
                          % (sections_get_id_json['code']))
            self.error = sections_get_id_json['code']
            return self.error

        logging.info("phpipam.sections_get_id: success %s"
                     % (sections_get_id_json['success']))
    ...
    def sections_get(self, section_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "sections/%s/?links=false"
                         % (section_id), headers=headers, verify=False)
        ...
        if not sections_get['success']:
            logging.error("phpipam.sections_get: FAILURE: %s"
                          % (sections_get['code']))
            self.error = sections_get['code']
            return self.error

        logging.info("phpipam.sections_get: success %s"
                     % (sections_get['success']))
    ...
    def sections_get_subnets(self, section_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "sections/%s/subnets/?links=false"
                         % (section_id), headers=headers, verify=False)
        sections_get_subnets = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.sections_get_subnets: Failure %s"
                          % (p.status_code))
        ...
        if not sections_get_subnets['success']:
            logging.error("phpipam.sections_get_subnets: FAILURE: %s"
                          % (sections_get_subnets['code']))
            self.error = sections_get_subnets['code']
            return self.error

        logging.info("phpipam.sections_get_subnets: success %s"
                     % (sections_get_subnets['success']))
    ...
    def sections_create(self, section_id, masterSection=0):
        ...
        p = requests.post(self.base + "sections/", headers=headers,
                          data=data, verify=False)
        sections_create = json.loads(p.text)

        if p.status_code != 201:
            logging.error("phpipam.sections_create: Failure %s"
                          % (p.status_code))
        ...
        if not sections_create['success']:
            logging.error("phpipam.sections_create: FAILURE: %s"
                          % (sections_create['code']))
            self.error = sections_create['code']
            return self.error

        logging.info("phpipam.sections_create: success %s"
                     % (sections_create['success']))
    ...
    def sections_delete(self, section_id,):
        headers = {'token': self.token}
        p = requests.delete(self.base + "sections/%s/"
                            % (section_id), headers=headers, verify=False)
        print p.text
        sections_delete = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.sections_delete: Failure %s"
                          % (p.status_code))
        ...
        if not sections_delete['success']:
            logging.error("phpipam.sections_delete: FAILURE: %s"
                          % (sections_delete['code']))
            self.error = sections_delete['code']
            return self.error

        logging.info("phpipam.sections_delete: success %s"
                     % (sections_delete['success']))
    ...
    def subnet_get(self, subnet_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "subnets/%s/?links=false"
                         % (subnet_id), headers=headers, verify=False)
        ...
        if not subnet_get['success']:
            logging.error("phpipam.subnet_get: FAILURE: %s"
                          % (subnet_get['code']))
            self.error = subnet_get['code']
            return self.error

        logging.info("phpipam.subnet_get: success %s"
                     % (subnet_get['success']))
    ...
    def subnet_search(self, subnet_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "subnets/cidr/%s/?links=false"
                         % (subnet_id), headers=headers, verify=False)
        subnet_search = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.subnet_search: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.subnet_search: success %s"
                     % (subnet_search['success']))
    ...
    def subnet_all(self, subnet_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "subnets/%s/addresses/?links=false"
                         % (subnet_id), headers=headers, verify=False)
        subnet_all = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.subnet_all: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.subnet_all: success %s"
                     % (subnet_all['success']))
    ...
    def subnet_first_available(self, subnet_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "subnets/%s/first_free/?links=false"
                         % (subnet_id), headers=headers, verify=False)
        subnet_first_available = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.subnet_first_available: Failure %s"
                          % (p.status_code))
        ...
        if not subnet_first_available['success']:
            logging.error("phpipam.subnet_first_available: FAILURE: %s"
                          % (subnet_first_available['code']))
            self.error = subnet_first_available['code']
            return self.error

        logging.info("phpipam.subnet_first_available: success %s"
                     % (subnet_first_available['success']))
    ...
    def subnet_create(self, subnet, mask, sectionId, description="",
                      vlanid=None, mastersubnetid=0, nameserverid=None):
        headers = {'token': self.token}
        data = {
            'subnet': subnet,
            'mask': mask,
        ...
        p = requests.post(self.base + "subnets/", headers=headers,
                          data=data, verify=False)
        subnet_first_available = json.loads(p.text)
    ...
    def subnet_delete(self, subnet_id, ):
        headers = {'token': self.token}
        p = requests.delete(self.base + "subnets/%s/"
                            % (subnet_id), headers=headers, verify=False)
        print p.text
        subnets_delete = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.subnets_delete: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.subnets_delete: success %s"
                     % (subnets_delete['success']))
    ...
    def address_get(self, address_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "addresses/%s/?links=false"
                         % (address_id), headers=headers, verify=False)
        ...
        if not address_get['success']:
            logging.error("phpipam.address_get: FAILURE: %s"
                          % (address_get['code']))
            self.error = address_get['code']
            return self.error

        logging.info("phpipam.address_get: success %s"
                     % (address_get['success']))
    ...
    def address_search(self, address):
        headers = {'token': self.token}
        p = requests.get(self.base + "addresses/search/%s/?links=false"
                         % (address), headers=headers, verify=False)
        address_get = json.loads(p.text)
    ...
    def address_create(self, ip, subnetId, hostname, description="",
                       is_gateway=0, mac=""):
        headers = {'token': self.token}
        data = {
            "ip": ip,
        ...
        p = requests.post(self.base + "addresses/", headers=headers,
                          data=data, verify=False)
        address_create = json.loads(p.text)

        if p.status_code != 201:
            logging.error("phpipam.address_create: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.address_create: success %s"
                     % (address_create['success']))
    ...
    def vlan_get_all(self):
        headers = {'token': self.token}
        p = requests.get(self.base + "vlan/?links=false",
                         headers=headers, verify=False)
        vlan_get_all_json = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.vlan_get_all: Failure %s"
                          % (p.status_code))
            logging.error(vlan_get_all_json)
            self.error = p.status_code
            self.error_message = vlan_get_all_json['message']
            return self.error, self.error_message

        if not vlan_get_all_json['success']:
            logging.error("phpipam.vlan_get_all: FAILURE: %s"
                          % (vlan_get_all_json['code']))
            self.error = vlan_get_all_json['code']
            return self.error

        logging.info("phpipam.sections_get_all: success %s"
                     % (vlan_get_all_json['success']))
        return vlan_get_all_json
    ...
    def vlan_get(self, vlan_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "vlans/%s/?links=false"
                         % (vlan_id), headers=headers, verify=False)
        vlan_get = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.vlan_get: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.vlan_get: success %s"
                     % (vlan_get['success']))
    ...
    def vlan_get_id(self, vlan_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "vlans/search/%s/?links=false"
                         % (vlan_id), headers=headers, verify=False)
        vlan_get = json.loads(p.text)
    ...
    def vlan_subnets(self, vlan_id):
        headers = {'token': self.token}
        p = requests.get(self.base + "vlans/%s/subnets/?links=false"
                         % (vlan_id), headers=headers, verify=False)
        print p.text
        vlan_subnets = json.loads(p.text)

        if p.status_code != 200:
            logging.error("phpipam.vlan_subnets: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.vlan_subnets: success %s"
                     % (vlan_subnets['success']))
    ...
    def vlan_create(self, number, name, description=""):
        ...
        p = requests.post(self.base + "vlans/", headers=headers,
                          data=data, verify=False)
        vlan_create = json.loads(p.text)

        if p.status_code != 201:
            logging.error("phpipam.vlan_create: Failure %s"
                          % (p.status_code))
        ...
        logging.info("phpipam.vlan_create: success %s"
                     % (vlan_create['success']))
    ...
    def vlan_delete(self, vlan_id, ):
        headers = {'token': self.token}
        p = requests.delete(self.base + "vlans/%s/"
                            % (vlan_id), headers=headers, verify=False)
        print p.text
        ...
        if not vlans_delete['success']:
            logging.error("phpipam.vlans_delete: FAILURE: %s"
                          % (vlans_delete['code']))
            self.error = vlans_delete['code']
            return self.error

        logging.info("phpipam.vlans_delete: success %s"
                     % (vlans_delete['success']))
